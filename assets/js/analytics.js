...
const SERVICE_ORDER = ["Teeth Cleaning", "Root Canal", "Braces"];
...
function renderLineChart(svg, values) {
  ...
  svg.innerHTML = `
    ...
    <polyline points="${polyline}" fill="none" stroke="#0f8f68" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"></polyline>
    ${points.map((point) => `
      <g>
        <circle cx="${point.x}" cy="${point.y}" r="5" fill="#0f8f68"></circle>
