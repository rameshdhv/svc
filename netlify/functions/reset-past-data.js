...
exports.handler = async (event) => {
  ...
  const { error: visitorError } = await adminSupabase
    .from("visitor_events")
    .delete()
    .lt("created_at", cutoffDate);
  if (visitorError) {
    return { statusCode: 500, body: JSON.stringify({ error: visitorError.message }) };
  }

  const { error: blockedSlotError } = await adminSupabase
    .from("blocked_slots")
    .delete()
    .lt("block_date", cutoffDate);
  if (blockedSlotError) {
    return { statusCode: 500, body: JSON.stringify({ error: blockedSlotError.message }) };
  }

  const { error: blockedDateError } = await adminSupabase
    .from("blocked_dates")
    .delete()
    .lt("to_date", cutoffDate);
  if (blockedDateError) {
    return { statusCode: 500, body: JSON.stringify({ error: blockedDateError.message }) };
  }
  ...
  return {
    ...
    body: JSON.stringify({
      ok: true,
      message: `Past data before ${cutoffMonth} deleted successfully.`
    })
  };
};
